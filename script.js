...
function renderProducts(products) {
    ...
    products.forEach(product => {
        ...
        card.innerHTML = `
            ...
            <p>Price: $${product.price.toFixed(2)} <span style="text-decoration:line-through;">$${product.strikePrice.toFixed(2)}</span></p>
...
function showDetail(productId) {
    ...
    detailContainer.innerHTML = `
        ...
        <p>Category: ${product.category}</p>
        <h3>Reviews:</h3>
        ${getReviewsForProduct(productId).map(review => `<p>${review.content} (Rating: ${review.rating})</p>`).join('')}
    `;
}
